...
async def login(user: UserLogin):
    ...
    try:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            await cur.execute("SELECT u.id, u.full_name, u.email, u.password_hash, u.role, u.phone, u.loyalty_points, b.id as barber_id, b.is_online, b.rating, b.specialization, b.bio, b.avatar_url, b.working_hours_start, b.working_hours_end FROM users u LEFT JOIN barbers b ON u.id = b.user_id WHERE u.email=%s", (user.email,))
